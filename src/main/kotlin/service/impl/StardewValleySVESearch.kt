...
object StardewValleySVESearch: WikiSearchCommand() {
    ...
    override val command = "xs"

    override val name = "星露谷SVE模组Wiki"
    ...
    override suspend fun search(word: String): JSONObject {
        val params = mapOf(
            "action" to "query",
            "list" to "search",
            "srsearch" to "SVE:$word",
            "format" to "json"
        )
        val result = getRequest(apiUrl, params)
        return JSONObject.parseObject(result?.replace("SVE:",""))
    }
}